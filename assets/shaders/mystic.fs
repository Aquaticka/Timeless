...
extern MY_HIGHP_OR_MEDIUMP vec2 mystic;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP number fac = 0.8 + 0.9*sin(13.*uv.x+5.32*uv.y + mystic.r*12. + cos(mystic.r*5.3 + uv.y*4.2 - uv.x*4.));
    MY_HIGHP_OR_MEDIUMP number fac2 = 0.5 + 0.5*sin(10.*uv.x+2.32*uv.y + mystic.r*5. - cos(mystic.r*2.3 + uv.x*8.2));
    MY_HIGHP_OR_MEDIUMP number fac3 = 0.5 + 0.5*sin(12.*uv.x+6.32*uv.y + mystic.r*6.111 + sin(mystic.r*5.3 + uv.y*3.2));
    MY_HIGHP_OR_MEDIUMP number fac4 = 0.5 + 0.5*sin(4.*uv.x+2.32*uv.y + mystic.r*8.111 + sin(mystic.r*1.3 + uv.y*13.2));
    MY_HIGHP_OR_MEDIUMP number fac5 = sin(0.5*16.*uv.x+5.32*uv.y + mystic.r*12. + cos(mystic.r*5.3 + uv.y*4.2 - uv.x*4.));

    MY_HIGHP_OR_MEDIUMP number maxfac = 0.6*max(max(fac, max(fac2, max(fac3,0.0))) + (fac+fac2+fac3*fac4), 0.);

    tex.rgb = tex.rgb*0.5 + vec3(0.8, 0.4, 0.5);
